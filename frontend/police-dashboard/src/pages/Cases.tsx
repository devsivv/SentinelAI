...
export default function Cases() {
  ...
  const fetchCases = useCallback(() => {
    ...
    caseService
      ...
      .catch((err: Error) => {
        ...
        // Do NOT inject mock data on error — show empty list with error banner
        setCasesList([]);
      })
      .finally(() => {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Empty workspace state — backend online but no cases yet */}
      {!isLoading && !errorVariant && casesList.length === 0 && (
        <div className="flex flex-col items-center justify-center rounded-xl border border-dashed border-gray-300 bg-white p-12 sm:p-16 text-center shadow-sm">
          <div className="mb-4 p-4 bg-gray-100 rounded-full">
            <FolderOpen className="h-8 w-8 text-gray-400" aria-hidden="true" />
          </div>
          <h3 className="text-lg font-semibold text-gray-900 mb-1">No investigations available</h3>
          ...
          <button
            onClick={fetchCases}
            className="inline-flex items-center gap-2 rounded-md bg-blue-600 px-4 py-2 text-sm font-medium text-white shadow-sm hover:bg-blue-700 focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-600 focus-visible:ring-offset-2 transition-colors min-h-[44px]"
          >
      ...
      {/* Cases table — only when data exists */}
      {!isLoading && !errorVariant && casesList.length > 0 && (
